...
<jsp:include page="/WEB-INF/include/adminNav.jsp" />
	...
	<ul class="nav nav-tabs" role="tablist">
		...
		<li class="nav-item dropdown">
			<a class="nav-link dropdown-toggle active" href="" data-toggle="dropdown">레시피 판매량 차트</a>
			<div class="dropdown-menu">
	...
	<table class="table table-hover">
		...
		<tbody id="sellRecipeWeekChartTable">
...
function weekCheck(date, title) {
	...
	if(targetDate == null) {
		...
		targetTitle = year + "년 " + month + "월 " + date + "일이 속한 주의 TOP10 레시피";
	}
	
	var we_params = { order_ymd : targetDate };
	...
	$("#sellRecipeWeekChartTable").empty();
	
	$.ajax({
		url: "getWeekTopSellRecipeChart.rk.ad",
		type: "POST",
		data: we_params,
		dataType: "JSON",
		success: function(result) {
			$.each(result, function(index, value){
				...
				$("#sellRecipeWeekChartTable").append(row);
			});	
...
$(function() {	
	...
	.on("changeDate", function() {
		...
		var month = arr[1];
		var day = arr[2];
		var title = year + "년 " + month + "월 " + date + "일이 속한 주의 TOP10 레시피";
		weekCheck(date, title);
	});
});//ready end
